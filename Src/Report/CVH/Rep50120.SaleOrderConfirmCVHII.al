report 50120 "SaleOrderConfirmCVH II"
{
    ApplicationArea = all;
    Caption = 'SaleOrderConfirmCVH II';
    RDLCLayout = './LayoutReport/StandardSalesOrderConfII.rdl';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Header; "Sales Header")
        {
            ...
            //Khmer label
            column(ItemNo_KHM; ItemNo_KHM) { }
            column(Description_KHM; Description_KHM) { }
            column(Qty_KHM; Qty_KHM) { }
            column(UOM_KHM; UOM_KHM) { }
            column(Price_KHM; Price_KHM) { }
            column(Discount_KHM; Discount_KHM) { }
            column(Total_KHM; Total_KHM) { }
            column(Picture_KHM; Picture_KHM) { }
            column(Note_KHM; Note_KHM) { }
            ...
            dataitem(Line; "Sales Line")
            {
                ...
                dataitem("BOM Component"; "BOM Component")
                {
                    ...
                    column(Bom_Type; Type) { }
                    column(Bom_Item_No; "No.") { }
                    column(Bom_Description; Description) { }
                    column(Bom_Quantity_per; "Quantity per") { }
                    column(Bom_Unit_of_Measure_Code; "Unit of Measure Code") { }
                    dataitem(Item_BOM; Item)
                    {
                        DataItemLink = "No." = FIELD("No.");
                        DataItemLinkReference = "BOM Component";
                        column(Unit_Price_BOMs; "Unit Price") { }
                    }
                }
                dataitem(Item; Item)
                {
    ...
    local procedure SetDefaultCaption()
    begin
        KhmerLable.Reset();
        if KhmerLable.FindSet() then
            repeat
                if KhmerLable.Code = 'DISCOUNT' then
                    Discount_KHM := KhmerLable."Khmer Description";
                if KhmerLable.Code = 'ITEMNO' then
                    ItemNo_KHM := KhmerLable."Khmer Description";
                if KhmerLable.Code = 'DESCRIPTION' then
                    Description_KHM := KhmerLable."Khmer Description";
                if KhmerLable.Code = 'QTY' then
                    Qty_KHM := KhmerLable."Khmer Description";
                if KhmerLable.Code = 'UOM' then
                    UOM_KHM := KhmerLable."Khmer Description";
                if KhmerLable.Code = 'PRICE' then
                    Price_KHM := KhmerLable."Khmer Description";
                if KhmerLable.Code = 'TOTAL' then
                    Total_KHM := KhmerLable."Khmer Description";
                if KhmerLable.Code = 'PICTURE' then
                    Picture_KHM := KhmerLable."Khmer Description";
                if KhmerLable.Code = 'NOTE' then
                    Note_KHM := KhmerLable."Khmer Description";
                if KhmerLable.code = 'DISCOUNT' then
                    Discount_KHM := KhmerLable."Khmer Description";
            until KhmerLable.Next() = 0;
    end;
    ...
    var
        ...
        CurrCode: Text[10];
        CurrSymbol: Text[10];
        KhmerLable: Record KhmerLabel;
        ItemNo_KHM: Text[100];
        Discount_KHM: Text[100];
        Discount_ENG: Text[100];
        Description_KHM: Text[100];
        Picture_KHM: Text[100];
        Qty_KHM: Text[100];
        UOM_KHM: Text[100];
        Total_KHM: Text[100];
        Price_KHM: Text[100];
    ...
    //pel vela der lern nas tae min arch bontor sneha jivit komdor enka kr nv jam sneha 
    ...
    procedure SetRequestOrderNo(OderNo: Code[24])
    begin
        orderNo := OderNo;
    end;
