report 50104 SalesInvoiceCVH
{
    RDLCLayout = './LayoutReport/StandardSalesInvoice.rdl';
    Caption = 'Sales Invoice CVH';
    DefaultLayout = RDLC;
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            column(Buyer_KHM; Buyer_KHM) { }
            column(Seller_KHM; Seller_KHM) { }
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(AmountIncludeVat; AmountIncludeVat)
                    {
                        Caption = 'Amount Include VAT';
                        ApplicationArea = Basic, Suite;
                        Editable = false;
                    }
                    // field(CheckVatamount; CheckVatamount())
                    // {
                    //     ApplicationArea = Basic, Suite;
                    // }
    ...
    local procedure CheckVatamount(): Text[20]
    var
        saleheader: Record "Sales Invoice Header";
    begin
        if saleheader."Amount Including VAT" > saleheader.amount then
            AmountIncludeVat := 'Yes'
        else
            AmountIncludeVat := 'No';
    end;
    ...
    begin
        ...
        if KhmerLable.FindSet() then
            repeat
                ...
                if KhmerLable.Code = 'SELLER' then
                    Seller_KHM := KhmerLable."Khmer Description";
                if KhmerLable.Code = 'BUYER' then
                    Buyer_KHM := KhmerLable."Khmer Description";
